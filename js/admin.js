...
async function loadDashboard() {
  ...
  try {
    ...
    const [
      ...
      totalPkgRes, profilesIncomeRes,
      todayDepRes, todayPkgRes, todayActRes
    ] = await Promise.all([
      ...
      db.from('package_purchases').select('amount'),
      db.from('profiles').select('total_income, today_income'),
      db.from('deposits').select('amount').eq('status','approved').gte('created_at', todayStart),
      db.from('package_purchases').select('amount').gte('purchased_at', todayStart),
      db.from('activities').select('amount').gte('created_at', todayStart)
    ]);
    ...
    const pkgData = totalPkgRes.data || [];
    const totalPkgCount = pkgData.length;
    const totalPkgAmount = pkgData.reduce((s,r) => s + parseFloat(r.amount||0), 0);

    const profilesData = profilesIncomeRes.data || [];
    const totalUserIncomeAllTime = profilesData.reduce((s,r) => s + parseFloat(r.total_income||0), 0);
    const totalUserIncomeToday = profilesData.reduce((s,r) => s + parseFloat(r.today_income||0), 0);

    const todayDep = (todayDepRes.data || []).reduce((s,r) => s + parseFloat(r.amount||0), 0);
    const todayPkg = (todayPkgRes.data || []).reduce((s,r) => s + parseFloat(r.amount||0), 0);
    const todayAct = (todayActRes.data || []).reduce((s,r) => s + Math.abs(parseFloat(r.amount||0)), 0);

    // 1. Outgoing Income: Processed Withdrawals + Total User Earnings Distributed
    const outgoingIncome = totalWd + totalUserIncomeAllTime;

    // 2. Today Total Business: Turnover generated today (Today Approved Deposits + Today Package Purchases + Today Transactions/Incomes)
    const todayTotalBusiness = todayDep + todayPkg + Math.max(todayAct, totalUserIncomeToday);

    // 3. Total Business (All Time): Total turnover / volume from Day 1 to current day (All Deposits + All Package Purchases + All User Incomes)
    const totalBusinessAllTime = totalDep + totalPkgAmount + totalUserIncomeAllTime;
    ...
    setStatCard('statOutgoingIncome', '$' + fmt(outgoingIncome));
    setStatCard('statTotalPkg',       totalPkgCount);
    setStatCard('statTodayBusiness',  '$' + fmt(todayTotalBusiness));
    setStatCard('statTotalBusiness',  '$' + fmt(totalBusinessAllTime));
